...
adsl %>% xportr_df_varnames("message")
```
...
For strings containing variable names `xportr_tidy_names()` was designed to identify and rename variables into compliance. Thus, you can use this function to rename the spec file variables or column names on the fly. Below, we use it's default options to tidy names for submission compliance.
```{r}
renamed_var_spec <- var_spec %>%
  filter(dataset == "ADSL") %>%
  mutate(tidy_variable = xportr_tidy_names(variable)
  ) %>%
...
The function `xportr_df_varnames()` takes `xportr_tidy_names` a step further to help users change the data.frame columns directly. It also is slightly less flexible from a customization perspective, attempting to follow the submission constraints precisely where as `xportr_tidy_names` can be used for a wider breadth of renaming applications. Executing the code below, we create a new data.frame called `adsl_renamed`, but the variable names are already compliant. The `identical` function below shows us that nothing has changed.
```{r, eval = TRUE}
adsl_renamed <- adsl %>% xportr_df_varnames()
identical(adsl, adsl_renamed)
```


In the interest of showcasing `xportr_df_varnames` capabilities, we create a fictional data.frame `adxx` riddled with non-compliant variable names below. In fact, `adxx` only has one valid variable name: "STUDYID". Calling the function shows this data.frame's variables violate all four compliance rules and proceeds to rename all but "STUDYID".
...
xportr_df_varnames(adxx) # default behavior
```
...
Please review the documentation using `?xportr_tidy_names` OR `?xportr_df_varnames` to learn how the abbreviation algorithm works and to further customize the renaming of your variable names using a slurry of additional arguments.
...
adsl %>%
  ...
  xportr_df_varnames("message") %>%
  xportr_write("adsl.xpt")
```

Optionally, leave out xportr_df_varnames and instead use `tidy_names = TRUE` in xportr_write() to accept the recommended defaults of `xportr_tidy_names`.

```{r, eval=FALSE}
# No xportr_df_varnames()!
adsl %>%
  xportr_type(var_spec, "ADSL", "message") %>%
  xportr_length(var_spec, "ADSL", "message") %>%
  xportr_label(var_spec, "ADSL", "message") %>%
  xportr_df_label(data_spec, "ADSL") %>%
  xportr_write("adsl.xpt", tidy_names = TRUE)
```
