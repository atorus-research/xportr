...
test_that("xpt_validate_var_names Test 6: utils-Get error message when the variable contains non-ASCII characters or underscore", {
  expect_equal(
...
test_that("xpt_validate Test 10: utils-Get error message when the label contains non-ASCII, symbol or special characters", {
  df <- data.frame(A = 1, B = 2)
...
test_that("xpt_validate Test 12: utils-Get error message when the length of a non-ASCII character variable is > 200 bytes", {
